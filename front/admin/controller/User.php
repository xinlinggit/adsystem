...
//        $row = UserModel::where('id', ADMIN_ID)->field('username,nick,email,mobile')->find()->toArray();
        ...
        $row = $UserModel->get_info();
		$this->assign('update_money', \think\Config::get('update_money'));
        $this->assign('data_info', $row);
        return $this->fetch();
    }
	 ...
	 */
    public function updateUserRole($id = 0)
    {
    	$role_auth = Db::table('ad_admin_role')->where('id = ' . $id)->value('auth');
	    $user_ids = Db::table('ad_admin_user')->field('id')->where('role_id = ' . $id)->select();
	    $ids = [];
	    foreach ($user_ids as $k => $v){
			array_push($ids, $v['id']);
	    }
	    $res = Db::table('ad_admin_user')->where('id', 'in', $ids)->update(['auth' => $role_auth]);
	    if($res !== false)
	    {
	    	return $this->success('下发权限成功');
	    } else {
	    	return $this->error('网络不稳定，请稍后尝试。');
	    }
    }
}
