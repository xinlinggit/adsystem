...
<!--			<a href="{:url('add')}" class="layui-btn layui-btn-primary"><i class="aicon ai-tianjia"></i>新增广告</a>-->
			<a href="{:url('addinfo')}" class="layui-btn layui-btn-primary"><i class="aicon ai-tianjia"></i>新增信息流</a>
	...
	<div class="layui-form">
		<table class="layui-table mt10" lay-even="" lay-skin="row">
			...
			<tr>
				...
				<th  style="text-align: center;" class="w100">所属广告位</th>
			...
			<tr>
                ...
                {if condition="$vo['adv_type'] eq '1'"}
                <td style="text-align: center;">信息流广告位</td>
                {else /}
                <td style="text-align: center;">{$vo['sensename']}</td>
                {/if}
				<td style="text-align: center;">{$vo['sitename']}</td>
				<td style="text-align: center;">{$vo['materialid']}</td>
                {if condition="$vo['adv_type'] eq '1'"}
                <td style="text-align: center;">信息流尺寸</td>
                {else /}
                ...
                <td style="text-align: center;">{if condition="$vo.running_status eq 1"}启用{else /}停用{/if}</td>
				<td style="text-align: center;">
					{switch name="$vo.status"}
						{case value="1"}暂停{/case}
						{case value="2"}即将投放{/case}
						{case value="3"}投放中{/case}
						{case value="4"}已投完{/case}
					{/switch}
				</td>
				<td style="text-align: center;">
					{$vo.create_time}
				...
				<td style="text-align: center;">
                    ...
                    {/if}
                    <a href="{:url('changerunningstatus?id='.$vo['id'].'&running_status='.$vo['running_status'])}" class="layui-btn layui-btn-primary">{if condition="$vo.running_status eq 2"}启用{else/} 停止 {/if} </a>
					<a href="{$review_url . $vo['materialid']}" target="_blank" class="layui-btn layui-btn-primary">预览</a>
                    <a href="{:url('del?id='.$vo['id'])}" class="layui-btn layui-btn-primary j-tr-del" reresh="yes">删除</a>
				</td>
			</tr>
			{/volist}
			</tbody>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <select name="status" lay-verify = "required" type="select">
                        ...
                        <option value="4" {if condition="input('get.status') eq 4"} selected {/if}>已投完</option>
                    </select>
                </div>
            </div>
            <div class="layui-form-item">
...
<script type="text/javascript">
    layui.use(['jquery', 'element', 'layer'], function() {
        ...
        $('.add_my_tab').click(function(event) {
            var that = $(this);
