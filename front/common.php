...
if(!function_exists('upload2fileserver'))
{
	 ...
	 */
	function upload2fileserver($path, $retry = 3, $sub = 'material')
	{
		// 将本地途径转换绝对路径
		...
		if($res)
		{
			return $Cfg['ftp_server_domain'] . DS . 'as' . DS . $subdir . DS . $filename;
		} else {
			return false;
		}
	}
...
// TODO: 1.在 mkdir 创建日志目录失败的时候记录日志 2.配置让有邮件发送代替日志记录是可选的
if(!function_exists('ads_log'))
{
	/**
	 * @param array $msg 消息
	 * @param int $front_or_cms 日志来源 1：广告系统前台  2: 广告系统后台
	 */
	function ads_log($msg = [], $front_or_cms = 1)
	{   @mkdir(LOG_PATH . '/' . date('Y') . '/' . date('md'), 0755, true);
		if($front_or_cms == 1)
		{
			@error_log(date('Y-m-d H:i:s') . ' |-info-| ' . print_r($msg, true) . PHP_EOL, 3, LOG_PATH . '/' . date('Y') . '/' . date('md') . '/ads_front_' . date('Ymd') . '.log');
		} else {
			@error_log(date('Y-m-d H:i:s') . ' |-info-| ' . print_r($msg, true) . PHP_EOL, 3, LOG_PATH . '/' . date('Y') . '/' . date('md') . '/ads_cms_' . date('Ymd') . '.log');
		}
	}
}
