...
class Generate
{
    ...
    public function run($data, $option = 'all')
    {
        ...
        if (isset($data['model']) && $data['model']) {
            $module = $this->readConfig($data['model'], 'app', 'model_path', Config::get('app.model_path'));
        ...
        if (isset($data['validate']) && $data['validate']) {
            $module = $this->readConfig($data['model'], 'app', 'validate_path', Config::get('app.validate_path'));
            $pathCheck = APP_PATH . $module . DS;
            if (!self::checkWritable($pathCheck)) {
                throw new Exception("目录没有权限不可写，请执行一下命令修改权限：<br>chmod -R 755 " . realpath($pathCheck), 403);
            }
        ...
        if (isset($data['model']) && $data['model']) {
            $dir_list[] = $this->module . DS . "model";
        }
        if (isset($data['validate']) && $data['validate']) {
            $dir_list[] = $this->module . DS . "validate" . DS . $this->dir;
        }
    ...
    private function buildIndex($path, $pathTemplate, $fileName, $tableName, $code, $data)
    {
        ...
        if (in_array("resume", $menuArr) || in_array("forbid", $menuArr)) {
            $tdMenu .= $this->tab(4) . '{$vo.state_text}' . "\n";
        }
        $tdMenu .= $this->tab(4) . '{tp:menu menu=\'sedit\' /}' . "\n";
        // 有回收站
        if (in_array('recyclebin', $menuArr)) {
            $form = '{include file="form" /}';
            $th = '{include file="th" /}';
            $td = '{include file="td" /}';
            $tdMenu .= $this->tab(4) . '{tp:menu menu=\'sdelete\' /}';
        } else {
            $form = implode("\n" . $this->tab(1), $code['search']);
            $th = implode("\n" . $this->tab(3), $code['th']);
            $td = implode("\n" . $this->tab(3), $code['td']);
            $tdMenu .= $this->tab(4) . '{tp:menu menu=\'sdeleteforever\' /}';
        }
    ...
    private function buildController($path, $pathTemplate, $fileName, $tableName, $code, $data)
    {
        ...
        return file_put_contents($file, str_replace(
                ["[MODULE]", "[TITLE]", "[NAME]", "[FILTER]", "[NAMESPACE]", "[TABLE]"],
                [$this->module, $this->data['title'], $this->name, $code['filter'], $this->namespaceSuffix, $this->data['table']],
                $template
            )
        );
    }
    ...
    private function buildModel($path, $pathTemplate, $fileName, $tableName, $code, $data)
    {
        ...
        if (isset($this->data['auto_timestamp']) && $this->data['auto_timestamp']) {
            $autoTimestamp = '// 开启自动写入时间戳字段' . "\n"
                . $this->tab(1) . 'protected $autoWriteTimestamp = \'int\';';
        }
    ...
    private function buildTable($path, $pathTemplate, $fileName, $tableName, $code, $data)
    {
        ...
        if (in_array('id', $auto_create_field)) {
            $fieldAttr[] = $this->tab(1) . "`id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '{$this->data['title']}主键'";
        }
        foreach ($this->data['field'] as $field) {
            if (!in_array($field['name'], $auto_create_field)) {
                // 字段属性
                $fieldAttr[] = $this->tab(1) . "`{$field['name']}` {$field['type']}"
            ...
            if (isset($field['key']) && $field['key'] && $field['name'] != 'id') {
                $key[] = $this->tab(1) . "KEY `{$field['name']}` (`{$field['name']}`)";
            }
        }
        ...
        if (isset($this->data['menu'])) {
            ...
            if (in_array("resume", $this->data['menu']) || in_array("forbid", $this->data['menu'])) {
                $fieldAttr[] = $this->tab(1) . "`status` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '状态，1-正常 | 0-禁用'";
            }
            ...
            if (in_array("delete", $this->data['menu']) || in_array("recyclebin", $this->data['menu'])) {
                ...
                $fieldAttr[] = $this->tab(1) . "`isdelete` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '删除状态，1-删除 | 0-正常'";
            }
        }
        ...
        if (isset($this->data['auto_timestamp']) && $this->data['auto_timestamp']) {
            ...
            $fieldAttr[] = $this->tab(1) . "`create_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '创建时间'";
            $fieldAttr[] = $this->tab(1) . "`update_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '更新时间'";
        }
        // 默认自动创建主键为id
        $fieldAttr[] = $this->tab(1) . "PRIMARY KEY (`id`)";
     ...
     * 用于缩进
     * @param int    $step  步数
     * @param string $string  填充符号
     * @param int    $size  步长
     * @return string
     */
    private function tab($step = 1, $string = ' ', $size = 4)
    {
    ...
    private function parseCode()
    {
            ...
            foreach ($this->data['form'] as $form) {
                ...
                if (isset($form['search']) && $form['search']) {
                    ...
                    switch ($form['search_type']) {
                        case 'select':
                            ...
                            // 默认选中
                            $searchSelected .= $this->tab(2) . '$("[name=\'' . $form['name'] . '\']").find("[value=\'{$Request.param.' . $form['name'] . '}\']").attr("selected", true);' . "\n";
                            $search[] = $this->tab(1) . '<div class="select-box" style="width:250px">';
                            $search[] = $this->tab(2) . '<select name="' . $form['name'] . '" class="select">';
                            $search = array_merge($search, $this->getOption($options, $form, true, 3));
                            $search[] = $this->tab(2) . '</select>';
                            $search[] = $this->tab(1) . '</div>';
                            break;
                        case 'date':
                            // td
                            $td[] = '<td>{$vo.' . $form['name'] . ($form['name'] == "status" ? '|get_status' : '') . '}</td>';
                            $search[] = $this->tab(1) . '<input type="text" class="input-text Wdate" style="width:250px" '
                        ...
                        default:
                            ...
                            if ($form['name'] == 'sort') {
                                ...
                                $td[] = '<td style="padding: 0">' . "\n"
                                    . $this->tab(1) . '<input type="number" name="sort[{$vo.id}]" value="{$vo.sort}" style="width: 60px;"' . "\n"
                                    . $this->tab(2) . 'class="input-text text-c order-input" data-id="{$vo.id}">'
                                    . '</td>';
                            } else {
                                $td[] = '<td>{$vo.' . $form['name'] . '|high_light=$Request.param.' . $form['name'] . "}</td>";
                            }
                            $filter .= $this->tab(2) . 'if ($this->request->param("' . $form['name'] . '")) {' . "\n"
                                . $this->tab(3) . '$map[\'' . $form['name'] . '\'] = ["like", "%" . $this->request->param("' . $form['name'] . '") . "%"];' . "\n"
                                . $this->tab(2) . '}' . "\n";
                            $search[] = $this->tab(1) . '<input type="text" class="input-text" style="width:250px" '
                                . 'placeholder="' . $form['title'] . '" name="' . $form['name'] . '" '
                                . 'value="{$Request.param.' . $form['name'] . '}" '
                                . '>';
                            break;
                ...
                if (!in_array($form['name'], ['id', 'isdelete', 'create_time', 'update_time'])) {
                    ...
                    if (isset($form['validate']) && $form['validate']['datatype']) {
                        ...
                        $validate .= $this->tab(2) . '"' . $form['name'] . '|' . $form['title'] . '" => "'
                            . (isset($form['require']) && $form['require'] ? 'require' : '') . '",' . "\n";
                    }
                    $editField .= $this->tab(2) . '<div class="row cl">' . "\n"
                        . $this->tab(3) . '<label class="form-label col-xs-3 col-sm-3">'
                        . (isset($form['require']) && $form['require'] ? '<span class="c-red">*</span>' : '')
                        . $form['title'] . '：</label>' . "\n"
                        . $this->tab(3) . '<div class="formControls col-xs-6 col-sm-6'
                    ...
                    switch ($form['type']) {
                        ...
                        case "checkbox":
                            if ($form['type'] == "radio") {
                                ...
                                $setChecked[] = $this->tab(2) . '$("[name=\'' . $form['name'] . '\'][value=\'{$vo.' . $form['name'] . ' ?? \'' . $form['default'] . '\'}\']").prop("checked", true);';
                            } else {
                                $setChecked[] = $this->tab(2) . 'var checks = \'' . $form['default'] . '\'.split(",");' . "\n"
                                    . $this->tab(2) . 'if (checks.length > 0){' . "\n"
                                    . $this->tab(3) . 'for (var i in checks){' . "\n"
                                    . $this->tab(4) . '$("[name=\'' . $form['name'] . '[]\'][value=\'"+checks[i]+"\']").prop("checked", true);' . "\n"
                                    . $this->tab(3) . '}' . "\n"
                                    . $this->tab(2) . '}';
                            }
                            ...
                            switch ($options[0]) {
                                case 'string':
                                    ...
                                    break;
                                case 'var':
                                    $editField .= $this->tab(4) . '{foreach name="$Think.config.conf.' . $options[1] . '" item=\'v\' key=\'k\'}' . "\n"
                        ...
                        case "select":
                            // select类型的控件进行编辑状态赋值
                            $setSelected[] = $this->tab(2) . '$("[name=\'' . $form['name'] . '\']").find("[value=\'{$vo.' . $form['name'] . ' ?? \'' . $form['default'] . '\'}\']").attr("selected", true);';
                            $editField .= $this->tab(4) . '<div class="select-box">' . "\n"
                                . $this->tab(5) . '<select name="' . $form['name'] . '" class="select"' . $validateForm . '>' . "\n"
                        ...
                        case "textarea":
                            ...
                            $editField .= $this->tab(4) . '<textarea class="textarea" placeholder="" name="' . $form['name'] . '" '
                                . 'onKeyUp="textarealength(this, 100)"' . $validateForm . '>'
                                . '{$vo.' . $form['name'] . ' ?? \'' . $form['default'] . '\'}'
                                . '</textarea>' . "\n"
                                . $this->tab(4) . '<p class="textarea-numberbar"><em class="textarea-length">0</em>/100</p>' . "\n";
                            break;
                        case "date":
                            $editField .= $this->tab(4) . '<input type="text" class="input-text Wdate" '
                                . 'placeholder="' . $form['title'] . '" name="' . $form['name'] . '" '
                        ...
                        case "number":
                        default:
                            $editField .= $this->tab(4) . '<input type="' . $form['type'] . '" class="input-text" '
                                . 'placeholder="' . $form['title'] . '" name="' . $form['name'] . '" '
                                . 'value="' . '{$vo.' . $form['name'] . ' ?? \'' . $form['default'] . '\'}' . '" '
                                . $validateForm . '>' . "\n";
                            break;
                    }
                    $editField .= $this->tab(3) . '</div>' . "\n"
                        . $this->tab(3) . '<div class="col-xs-3 col-sm-3"></div>' . "\n"
                        . $this->tab(2) . '</div>' . "\n";
                }
            }
        }
        if (count($search) > 1) {
            // 有设置搜索则显示
            $search[] = $this->tab(1) . '<button type="submit" class="btn btn-success"><i class="Hui-iconfont">&#xe665;</i> 搜索</button>';
            $search[] = '</form>';
        } else {
            // 不设置将form.html置空
            $search = [];
    ...
    {
        return $this->tab($tab) . '<div class="radio-box">' . "\n"
            . $this->tab($tab + 1) . '<input type="' . $form['type'] . '" name="' . $name . '" '
            . 'id="' . $form['name'] . '-' . $key . '" value="' . $value . '"' . $validateForm . '>' . "\n"
            . $this->tab($tab + 1) . '<label for="' . $form['name'] . '-' . $key . '">' . $title . '</label>' . "\n"
            . $this->tab($tab) . '</div>' . "\n";
    }
    ...
    private function getOption($options, $form, $empty = true, $tab = 3)
    {
        switch ($options[0]) {
            ...
            case 'var':
                $ret = [];
                if ($empty) {
                    $ret[] = $this->tab($tab) . '<option value="">所有' . $form['title'] . '</option>';
                }
                $ret[] = $this->tab($tab) . '{foreach name="$Think.config.conf.' . $options[1] . '" item=\'v\' key=\'k\'}';
                $ret[] = $this->tab($tab + 1) . '<option value="{$k}">{$v}</option>';
                $ret[] = $this->tab($tab) . '{/foreach}';
            ...
            case 'think_var':
                ...
                $ret[] = $this->tab($tab) . '{foreach name="$' . $options[1] . '" item=\'v\'}';
                $ret[] = $this->tab($tab + 1) . '<option value="{$v.id}">{$v.name}</option>';
                $ret[] = $this->tab($tab) . '{/foreach}';
            ...
            case 'array':
                $ret = [];
                foreach ($options[1] as $option) {
                    $ret[] = $this->tab($tab) . '<option value="' . $option[0] . '">' . $option[1] . '</option>';
                }
